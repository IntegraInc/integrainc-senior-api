...
export class AnalisysController {
 ...
 async getAnalisys_2(req: Request, res: Response) {
  const authHeader = req.headers.authorization;
  ...
  try {
   const result = await this.service.getAnalysis_3(
    username,
    password,
    0,
    limit,
  ...
  *         description: Internal error or SOAP failure
  */
 async postBuyingOrder_2(req: Request, res: Response) {
  try {
   const authHeader = req.headers.authorization;
   if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).json({
   ...
   const result = await this.service.sendBuyingOrder_2(
    username,
    password,
    req.body
   );
